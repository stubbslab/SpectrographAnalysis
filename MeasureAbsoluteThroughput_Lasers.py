...
if __name__=="__main__":
    ...
    all_bias_indeces = can.flattenListOfLists([[20 + i * 3, 20 + i * 3 + 2] for i in range(25)])

    wavelengths = [635, 650, 808, 850, 780]
    start_index = 62
    ...
    biases = [ 'Bias_' + '_'.join(date_strs) + '_' + str(index) + '.fits' for index in all_bias_indeces]
    ...
    print ('ref_spec_solution_file = ' + str(ref_spec_solution_file))
    if not(os.path.isfile(target_dir + ref_spec_solution_file)):
        redo_spec_solution = 1
    print ('redo_spec_solution = ' + str(redo_spec_solution))
    ...
    if not(redo_spec_solution):
        ...
        print ('processor.bg_std_buffer = ' + str(processor.bg_std_buffer))
    expected_pix_centers = [ int(processor.mu_of_wavelength_solution(wave)) for wave in wavelengths ]

    #Process spectral data
    redo_image_processing = 0
    laser_indeces = [57, 66, 72, 81, 93]
    laser_images = [ 'Laser' + str(wavelengths[i]) + '_f' + focus_str + '_' + '_'.join(date_strs) + '_' + str(laser_indeces[i]) + '.fits' for i in range(len(wavelengths)) ]
    processed_laser_images = ['proc_' + laser_image for laser_image in laser_images]
    #save_stacked_image_name = 'VLM635_f23p7_2021_05_29_MEDIAN.fits'
    #sum_box = [[866, 882], [450, 704]]
    #exp_time = 1.0
    for i in range(len(laser_images)):
        if not (os.path.isfile(target_dir + processed_laser_images[i])) or redo_image_processing:
            processor.processImages(spec_files_to_reduce = [laser_images[i]], do_bias = 1, do_dark = 1, crc_correct = 1, apply_background_correction = 0, redetermine_spec_range = 0, apply_scatter_correction = 0, save_stacked_image = 1, save_image_name = processed_laser_images[i] )
    ...
    show_snippings = 0
    for i in range(len(processed_laser_images)):
        region = regions[i]
        bg_region = bg_regions[i]
        image_data, header = can.readInDataFromFitsFile(processed_laser_images[i], target_dir)
        ...
        if show_snippings:
            ...
            axarr[0].imshow(data_snip)
            axarr[1].imshow(bg_snip)
            plt.draw()
            plt.pause(2)
            plt.close()
    ...
    PD_curr_data_file = 'Lasers_PD_1M_int_sphere_Data.txt'
    ...
    PD_dark_currs = [[[float(elem) for elem in sequence.split(',')] for sequence in PD_data[1]], [[float(elem) for elem in sequence.split(',')] for sequence in PD_data[4]]]
    PD_bright_currs = [[[float(elem) for elem in sequence.split(',')] for sequence in PD_data[2]], [[float(elem) for elem in sequence.split(',')] for sequence in PD_data[3]]]
    n_PD_data_subfigs = int(np.ceil(np.sqrt(len(PD_bright_currs[0]))))
    print ('len(PD_bright_currs[0]) = ' + str(len(PD_bright_currs[0])))
    print ('n_PD_data_subfigs = ' + str(n_PD_data_subfigs))
    ...
    for i in range(len(PD_bright_currs[0])):
        ax = bright_axarr[i // n_PD_data_subfigs, i % n_PD_data_subfigs]
        ax.plot(range(len(PD_dark_currs[0][i]) + 1, len(PD_dark_currs[0][i]) + 1 + len(PD_bright_currs[0][i]) ), PD_bright_currs[0][i], c = 'k')
        ax.plot(range(len(PD_dark_currs[0][i]) + 1 + len(PD_bright_currs[0][i]), len(PD_dark_currs[0][i]) + 1 + len(PD_bright_currs[0][i]) + len(PD_bright_currs[1][i]) ), PD_bright_currs[1][i], c = 'k')
        ax.axvline(len(PD_dark_currs[0][i]) + 1 + len(PD_bright_currs[0][i]), c = 'gray', linestyle = '--')
    ...
    for i in range(len(PD_dark_currs[0])):
        ...
        ax.plot(range(len(PD_dark_currs[0][i]) + 1 + len(PD_bright_currs[0][i]) + len(PD_bright_currs[1][i]) , len(PD_dark_currs[0][i]) + 1 + len(PD_bright_currs[0][i]) + len(PD_bright_currs[1][i])  + len(PD_dark_currs[1][i])), PD_dark_currs[1][i], c = 'k')
        ax.text(0.1, 0.9, r'$\lambda = $' + str(ref_wavelengths_nm[i]) + 'nm', c = 'g', transform=ax.transAxes)
        ax.set_xlabel('N in Sequence')
        ax.set_ylabel('PD current (A)')
    plt.savefig(target_dir + 'DarkCurrentDataStream.pdf')
    plt.close('all')
    PD_bright_means, PD_bright_uncertainties = [ [[ can.sigClipMean(curr_arr, sig_clip = 5) for curr_arr in PD_bright_currs[0]], [ can.sigClipMean(curr_arr, sig_clip = 3) for curr_arr in PD_bright_currs[1]]],
                      [ [ can.sigClipStd(curr_arr, sig_clip = 5, standard_error = 1) for curr_arr in PD_bright_currs[0]], [ can.sigClipStd(curr_arr, sig_clip = 3, standard_error = 1) for curr_arr in PD_bright_currs[1]] ] ]
    PD_dark_means, PD_dark_uncertainties = [ [[ can.sigClipMean(curr_arr, sig_clip = 5) for curr_arr in PD_dark_currs[0]], [ can.sigClipMean(curr_arr, sig_clip = 3) for curr_arr in PD_dark_currs[1]]],
                      [ [ can.sigClipStd(curr_arr, sig_clip = 5, standard_error = 1) for curr_arr in PD_dark_currs[0]], [ can.sigClipStd(curr_arr, sig_clip = 3, standard_error = 1) for curr_arr in PD_dark_currs[1]] ] ]

    dark_drift_uncertainty = can.sigClipStd(PD_dark_means[0] + PD_dark_means[1], sig_clip = 5, standard_error = 0)
    bright_drift_uncertainties = np.abs((np.array(PD_bright_means[1]) - np.array(PD_bright_means[0])))
    PD_currents_A = np.abs((np.array(PD_bright_means[0]) + np.array(PD_bright_means[1])) / 2 - (np.array(PD_dark_means[0]) + np.array(PD_dark_means[1])) / 2)
    PD_currents_A_uncertainty = np.sqrt( (np.array(PD_bright_uncertainties[0]) ** 2.0 + np.array(PD_bright_uncertainties[1]) ** 2.0) / 2 + bright_drift_uncertainties ** 2.0 + np.array(dark_drift_uncertainty) ** 2.0 + (np.array(PD_dark_uncertainties[0]) ** 2.0 + np.array(PD_dark_uncertainties[1]) ** 2.0) / 2 )
    ...
    axarr.set_xlabel('Calibration wavelength (nm)')
    axarr.set_ylabel(r'Throughput (ADU Ry $^{-1}$ s $^{-1}$)')
